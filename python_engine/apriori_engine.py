...
def run_apriori_analysis():
    ...
    if len(sys.argv) >= 3:
        try:
            ...
            print(f"Configuration loaded from command-line: min_support={min_support:.4f}, min_confidence={min_confidence:.4f}")
            read_db_settings = False
        except ValueError:
            print("Warning: Invalid command-line arguments. Falling back to system settings.", file=sys.stderr)

    print("Connecting to local database db_agri_dss...")
    try:
    ...
    if read_db_settings:
        try:
            ...
            print(f"Configuration loaded from database: min_support={min_support:.4f}, min_confidence={min_confidence:.4f}")
        except mysql.connector.Error as err:
            print(f"Warning: Could not read system settings. Using defaults. Details: {err}", file=sys.stderr)

    # Fetch clean dataset
    try:
        cursor.execute("SELECT barangay, crop_type, season, intervention_received FROM tbl_rsbsa_data")
    ...
    if not rows:
        print("Dataset is empty. Run data management CSV upload first.")
        cursor.close()
        conn.close()
        sys.exit(0)

    print(f"Retrieved {len(rows)} records. Constructing transaction list...")
    ...
    print("Mining frequent itemsets using Apriori algorithm...")
    try:
    ...
    if frequent_itemsets.empty:
        print("No itemsets found meeting the minimum support threshold.")
    ...
    print(f"Generated {len(rules)} association rules. Writing back to database...")
    try:
        ...
        conn.commit()
        print(f"Success! Inserted {len(insert_data)} association rules into tbl_forecast_rules.")
...
def clear_and_commit_rules(conn, cursor):
    ...
    try:
        cursor.execute("TRUNCATE TABLE tbl_forecast_rules")
        conn.commit()
        print("Truncated tbl_forecast_rules because no rules met the support/confidence thresholds.")
    except mysql.connector.Error as err:
